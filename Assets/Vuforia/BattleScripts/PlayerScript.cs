...
public class PlayerScript : MonoBehaviour {
	...
	private string[] squirtleAttacks = { "Water gun", "Bubbles", "Scratch" };

    public IEnumerator GetAttack(Pokemon pokemon)
    {
        while (attack == null) yield return null;
        ...
        if (result != null && result.results.Length > 0)
        {
            foreach (var res in result.results)
            {
                foreach (var alt in res.alternatives)
                {
					...
					if ((alt.transcript.Contains("use water gun") || alt.transcript.Contains("water")
                            || alt.transcript.Contains("use water guns") || alt.transcript.Contains("use water gun"))
							&& ResultsField.text.Contains("Final")) {
						attack = "Water gun";
            // squirtleAttack("Bubble");
            StartCoroutine(watergunz());

					}

					if ((alt.transcript.Contains("bubble") || alt.transcript.Contains("you bubbles")
                            || alt.transcript.Contains("use bubble") || alt.transcript.Contains("you bubbles"))
							&& ResultsField.text.Contains("Final")) {
						attack = "Bubbles";
            // squirtleAttack("Bubble");
            StartCoroutine(bubble());
					...
					if ((alt.transcript.Contains("scratch") || alt.transcript.Contains("you scratch")
                            || alt.transcript.Contains("use scratches") || alt.transcript.Contains("you scratches"))
							&& ResultsField.text.Contains("Final")) {
						attack = "Scratch";
            StartCoroutine(scratch());
            // squirtleAttack("Scratch");
					}
                }
